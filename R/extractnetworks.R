...
for (c in countrynames){
  ...
  if(any(pas@data$ISO3==c & c != "ATA")){
    ...
    ###test fast gdistance###
    combns <- t(combn(length(firstnet_laea),2))
    net_split <- split(firstnet_laea, seq_len(length(firstnet_laea))) 
    dists <- apply(combns,1, function(x)
      gDistance(net_split[[x[1]]], net_split[[x[2]]]))
    
    dist_table <- cbind.data.frame(from=row.names(firstnet_laea@data)[combns[, 1]], 
                                         to=row.names(firstnet_laea@data)[combns[, 2]], 
                                         d=dists)
    ...
    namepath <- paste(OUTconeforRfolder,paste(paste("distances/",c,sep=""),"firstnetworkdis2.txt",sep=""),sep="")
