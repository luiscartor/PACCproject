...
# The zero values of the original raster are NAs, so we set 0 to NA (and 32767)
...
for (c in countrynames){
  ...
  if(any(pas@data$ISO3==c & c != "ATA" & c != "MSR")){
    ...
    coun_stats <- data.frame(c,pa_mean,pa_med,pa_var,outpa_mean,outpa_med,outpa_var,
                    oldpa_mean,oldpa_med,oldpa_var,coun_mean,coun_med,coun_var)
...
colnames(stats_mat) <- c("country","pamean","pamed","pavar","outpamean","outpamed","outpavar",
                        "oldpamean","oldpamed","oldpavar","counmean","counmed","counvar")
